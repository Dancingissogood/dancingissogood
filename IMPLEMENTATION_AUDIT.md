...
- Ignores dependencies, env files, OS noise, build output, generated Next type env files, coverage, TypeScript build info, and logs.
...
`landing/next.config.ts` - 8 lines

- Enables `reactStrictMode`.
- Disables Next 16.3 agent-rule file generation so `next dev` does not create `AGENTS.md` and `CLAUDE.md`.
- Keeps framework config minimal aside from the confirmed generated-file opt-out.
...
- Ignored by Git because Next 16.3 rewrites it for `next dev`, `next build`, and `next typegen`.
- Should not be manually edited or committed.
